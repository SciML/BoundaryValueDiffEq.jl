...
@testset "Radau interpolations" begin
    for stage in (2, 3, 5, 7)
        ...
        @eval radau_solver(::Val{$stage}) = $(s)(NewtonRaphson(),BVPJacobianAlgorithm(AutoSparseFiniteDiff()); nested) #TODO: Change to forwarddiff when possible
    end
...
@testset "LobattoIII interpolations" begin for lobatto in ("a", "b", "c")
    for stage in (3, 4, 5)
        ...
        @eval lobatto_solver(::Val{$stage}) = $(s)(NewtonRaphson(),BVPJacobianAlgorithm(AutoSparseFiniteDiff()); nested) #TODO: Change to forwarddiff when possible
    end
