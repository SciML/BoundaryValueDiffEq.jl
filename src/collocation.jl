...
@inline function auto_diff_update_K!(S::BVPSystem, cache::MIRK4Cache, TU::MIRKTableau, i, h)
...
function auto_diff_Φ!(S::BVPSystem, TU::MIRKTableau, cache::MIRKCache)
    order, residual, N, y, x = S.order, S.residual, S.N, S.y, S.x
    K, b = cache.K, TU.b
    for i in 1:N-1
        h = x[i+1] - x[i]
        auto_diff_update_K!(S, cache, TU, i, h)
        # Update residual
