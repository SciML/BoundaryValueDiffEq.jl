...
function BVPSystem(fun, bc, p, x, M::Integer, order)
    ...
    return BVPSystem(order, M, N, fun, bc, p, x, y, vector_alloc(T, M, N), vector_alloc(T, M, N),
                     eltype(y)(undef, M))
end
...
function BVPSystem(fun, bc, p, x, y, order)
    ...
    M, N = size(y)
    return BVPSystem{T,U}(order, M, N, fun, bc, p, x, y, vector_alloc(T, M, N),
...
function BVPSystem(prob::BVProblem, x, order)
    ...
    N = size(x, 1)
    y = vector_alloc(T, M, N)
    return BVPSystem(order, M, N, prob.f, prob.bc, prob.p, x, y, vector_alloc(T, M, N),
                     vector_alloc(T, M, N), typeof(x)(undef, M))
end
