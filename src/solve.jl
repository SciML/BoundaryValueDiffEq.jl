...
function DiffEqBase.__solve(prob::BVProblem, alg::Union{GeneralMIRK, MIRK}; dt = 0.0, tol = 1e-6,
                            kwargs...)
    if dt <= 0
    ...
    while info == 0 && defect_norm > tol
        ...
        if info == 0
            defect, defect_norm, k_interp = defect_estimate(prob, Y, alg, n, dt, len, mesh, k_discrete)
...
function mesh_selector(mesh_current, defect, tol, n, len, alg)
    ...
    safety_factor = 1.3; rho = 1.0
    upper_new_mesh = 4.0; lower_new_mesh = 0.5
    r1 = 0.0; r2 = 0.0
    Nsub_star = 0
    info = 0
    p = alg_order(alg)
...
function defect_estimate(prob, Y, alg, n, dt, len, mesh, k_discrete)
    ...
    for i=1:n
        k_interp = interp_setup(mesh[i], dt, Y[i, :], Y[i+1, :], s, s_star, x_star, v_star, c_star, k_discrete[i, :], prob.f, len)

        # Sample point 1
        z, z_prime = sum_stages(weights_1, weights_1_prime, k_discrete, k_interp, len, dt, Y)
        ...
        # Sample point 2
        z, z_prime = sum_stages(weights_2, weights_2_prime, k_discrete, k_interp, len, dt, Y)
...
function interp_setup(tim1, hi, yim1, yi, s, s_star, x_star, v_star, c_star, ki_discrete, f, len)
    ...
    for r=1:s_star-s
        ...
        for j=1:s
            new_stages .= new_stages .+ x_star[j*(s_star-s)+r] .* ki_discrete[(j-1)*len+1]
        end
        for j=1:r-1
...
function interp_weights(tau, alg)
    ...
    elseif alg_order(alg) == 6
        w = [tau - 28607/7434*tau^2 - 166210/33453*tau^3 + 334780/11151*tau^4 - 1911296/55755*tau^5 + 406528/33453*tau^6,

              777/590*tau^2-2534158/234171*tau^3 + 2088580/78057*tau^4-10479104/390285*tau^5 + 11328512/1170855*tau^6,

              -1008/59*tau^2 + 222176/1593*tau^3 - 180032/531*tau^4 + 876544/2655*tau^5 - 180224/1593*tau^6,
                                                                        
              -1008/59*tau^2 + 222176/1593*tau^3 - 180032/531*tau^4 + 876544/2655*tau^5-180224/1593*tau^6,

              -378/59*tau^2 + 27772/531*tau^3 - 22504/177*tau^4 + 109568/885*tau^5 - 22528/531*tau^6,

              -95232/413*tau^2 + 62384128/33453*tau^3 - 49429504/11151*tau^4 + 46759936/11151*tau^5 - 46661632/33453*tau^6,

              896/5*tau^2 - 4352/3*tau^3 + 3456*tau^4 - 16384/5*tau^5 + 16384/15*tau^6,

              50176/531*tau^2 - 179554304/234171*tau^3 + 143363072/78057*tau^4 - 136675328/78057*tau^5 + 137363456/234171*tau^6,

              16384/441*tau^3-16384/147*tau^4 + 16384/147*tau^5 - 16384/441*tau^6]

#     Derivative polynomials.

        wp = [1 - 28607/3717*tau - 166210/11151*tau^2 + 1339120/11151*tau^3 - 1911296/11151*tau^4 + 813056/11151*tau^5,

           777/295*tau - 2534158/78057*tau^2 + 8354320/78057*tau^3 - 10479104/78057*tau^4 + 22657024/390285*tau^5,

           -2016/59*tau + 222176/531*tau^2 - 720128/531*tau^3 + 876544/531*tau^4 - 360448/531*tau^5,

           -2016/59*tau + 222176/531*tau^2 - 720128/531*tau^3 + 876544/531*tau^4 - 360448/531*tau^5,

           -756/59*tau + 27772/177*tau^2 - 90016/177*tau^3 + 109568/177*tau^4 - 45056/177*tau^5,

           -190464/413*tau + 62384128/11151*tau^2 - 197718016/11151*tau^3 + 233799680/11151*tau^4 - 93323264/11151*tau^5,

           1792/5*tau - 4352*tau^2 + 13824*tau^3 - 16384*tau^4 + 32768/5*tau^5 ,

           100352/531*tau - 179554304/78057*tau^2 + 573452288/78057*tau^3 - 683376640/78057*tau^4 + 274726912/78057*tau^5,

           16384/147*tau^2 - 65536/147*tau^3 + 81920/147*tau^4 - 32768/147*tau^5]
