...
@views function Φ!(residual, cache::MIRKNCache, y, u, p = cache.p)
    return Φ!(residual, cache.fᵢ_cache, cache.fᵢ₂_cache, cache.k_discrete, cache.f, cache.TU, y, u, p, cache.mesh, cache.mesh_dt, cache.stage)
end

@views function Φ!(residual, fᵢ_cache, fᵢ₂_cache, k_discrete, f!, TU::MIRKNTableau, y, u, p, mesh, mesh_dt, stage::Int)
    ...
    for i in 1:(L - 1)
        ...
        for r in 1:stage
            @. tmp = (1 - v[r]) * yᵢ + v[r] * yᵢ₊₁ + dtᵢ*((c[r] - v[r] - w[r]) * yₗ₊ᵢ + w[r] * yₗ₊ᵢ₊₁)
...
function Φ(cache::MIRKNCache, y, u, p = cache.p)
    return Φ(cache.fᵢ_cache, cache.fᵢ₂_cache, cache.k_discrete, cache.f, cache.TU, y, u, p, cache.mesh, cache.mesh_dt, cache.stage)
end

@views function Φ(fᵢ_cache, fᵢ₂_cache, k_discrete, f, TU::MIRKNTableau, y, u, p, mesh, mesh_dt, stage::Int)
    (; c, v, w, b, x, vp, bp, xp) = TU
