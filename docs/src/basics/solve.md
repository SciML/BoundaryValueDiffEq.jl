...
```
  ...
  - `ensemblealg`: Whether `MultipleShooting` parallelizes its internal ODE solves, default as `EnsembleThreads()`. The parallel solves run through KernelAbstractions on `MultipleShooting`'s `platform` backend. For more information, see the documentation for OrdinaryDiffEq: [EnsembleAlgorithms](https://docs.sciml.ai/DiffEqDocs/latest/features/ensemble/#EnsembleAlgorithms).
