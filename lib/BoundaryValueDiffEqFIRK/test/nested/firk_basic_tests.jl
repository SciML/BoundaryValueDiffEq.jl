...
@testitem "Test initial guess" begin
    ...
    sol1 = solve(bvp1, RadauIIa5(; nested_nlsolve = true), dt = 0.1, nlsolve_kwargs = (; maxiters = 0))
    @test sol1.u == u_guess

    bvp2 = TwoPointBVProblem(f!, (bca!, bcb!), sol1, tspan; bcresid_prototype = (zeros(2), zeros(1)))
    sol2 = solve(bvp2, RadauIIa5(; nested_nlsolve = true), dt = 0.1, nlsolve_kwargs = (; maxiters = 0))
    @test sol2.u == u_guess
end
