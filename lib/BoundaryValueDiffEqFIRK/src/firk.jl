@concrete struct FIRKCacheNested{iip, T, diffcache, tunable_parameters} <:
...
@concrete struct FIRKCacheExpand{iip, T, diffcache, tunable_parameters} <:
    AbstractBoundaryValueDiffEqCache
    order::Int                 # The order of FIRK method
    stage::Int                 # The state of FIRK method
    M::Int                     # The number of equations
...
function init_nested(
    ...
    K0 = __K0_on_u0(prob, stage; tunable_parameters = tunable_parameters)
    ...
    return FIRKCacheNested{iip, T, typeof(diffcache), tunable_parameters}(
...
function init_expanded(
    ...
    return FIRKCacheExpand{iip, T, typeof(diffcache), tunable_parameters}(
        alg_order(alg), stage, M, size(X), f, bc, prob_, prob.problem_type, prob.p,
        alg, TU, ITU, f_prototype, bcresid_prototype, mesh, mesh_dt, k_discrete,
...
function SciMLBase.solve!(
        cache::FIRKCacheExpand{
            iip, T, diffcache, tunable_parameters,
        }
    ) where {iip, T, diffcache, tunable_parameters}
...
function __construct_problem(
        cache::FIRKCacheExpand{iip, T, DC, tunable_parameters}, y, loss_bc::BC, loss_collocation::C,
        loss::LF, ::StandardBVProblem, ::Val{true}
    ) where {iip, T, DC, tunable_parameters, BC, C, LF}
...
function __construct_problem(
        cache::FIRKCacheNested{iip, T, DC, tunable_parameters}, y, loss_bc::BC, loss_collocation::C,
        loss::LF, ::StandardBVProblem, ::Val{true}
    ) where {iip, T, DC, tunable_parameters, BC, C, LF}
    (; jac_alg) = cache.alg
    (; bc_diffmode) = jac_alg
