...
function SciMLBase.solve!(cache::Union{FIRKCacheExpand, FIRKCacheExpand})
...
=#
    u = (cache isa FIRKCacheExpand) ? shrink_y([reshape(y, cache.in_size) for y in cache.y₀],
        length(cache.mesh), cache.M, alg_stage(cache.alg)) : recursivecopy(cache.y₀)
...
=#
function __perform_firk_iteration(cache::FIRKCacheNested, abstol,
...
function __construct_nlproblem(cache::Union{FIRKCacheExpand{iip}, FIRKCacheNested{iip}}, y::AbstractVector) where {iip}
