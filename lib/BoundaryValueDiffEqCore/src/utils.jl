...
function __extract_problem_details(
        ...
        check_positive_dt::Bool = false, tunable_parameters::Bool = false
    )
    ...
    if tunable_parameters
        prob.p isa SciMLBase.NullParameters &&
            throw(ArgumentError("`tunable_parameters` is true but `prob.p` is not set."))
...
function __extract_problem_details(
        ...
        tunable_parameters::Bool = false
    ) where {F <: Function}
    # Problem passes in a initial guess function
    check_positive_dt && dt ≤ 0 && throw(ArgumentError("dt must be positive"))

    u0 = __initial_guess(f, prob.p, prob.tspan[1]; tunable_parameters = tunable_parameters)
    t₀, t₁ = prob.tspan
...
function __initial_guess(f::F, p::P, t::T; tunable_parameters = false) where {F, P, T}
