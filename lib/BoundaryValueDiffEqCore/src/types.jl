...
"""
    __optimization_sparsity_detector(diffmode)

Sparsity detector for the optimization (BVP-with-cost) path. Unlike the nlsolve path,
which re-detects the pattern whenever the mesh changes, the optimization backends freeze
the constraint-Jacobian pattern once, at the initial guess, and refill the same nonzeros
every iterate. The pattern therefore has to be structural (input-independent): a local
detector probed at a degenerate initial guess records a strict subset of the runtime
pattern, and the refill at any physically active iterate overflows the declared nonzeros.
The local-tracer default is promoted to the global `TracerSparsityDetector`; any other
user-supplied detector is honored as is.
"""
