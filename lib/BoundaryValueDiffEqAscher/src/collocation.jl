function Φ!(cache::AscherCache{iip, T}, z, rhs) where {iip, T}
    ...
    for i=1:n
        ...
        for j=1:k
            ...
            @views vwblok(cache, xcol, hrho, j, w[i], v[i], ipvtw[i], uval, df, acol[:,j], dmzo[i])
        end
...
@inline __get_value(z::Vector{<:AbstractArray}) = eltype(first(z)) <: ForwardDiff.Dual ? [map(x->x.value, a) for a in z] : z
...
function Φ(cache::AscherCache{iip, T}, z) where {iip, T}
    ...
    for i=1:n
        ...
        for j=1:k
            ...
            # fill in ncy rows of  w and v
            @views vwblok(cache, xcol, hrho, j, w[i], v[i], ipvtw[i], zyval, df, acol[:,j], dmzo[i])
        end
...
function approx(cache::AscherCache{iip, T}, x, zval) where {iip, T}
    ...
    # evaluate z(u(x))
    for jcomp = 1:ncomp
        zsum = sum(a[j]*dmz[i][j][jcomp] for j=1:k)
...
function gblock(cache::AscherCache, h, irow, wi, vrhsz, rhsdmz, ipvtw)
    ...
    for jcomp=1:ncomp
        rhsz[irow+jcomp-1] = sum(hb[j]*rhsdmz[j][jcomp] for j=1:k)
    end
...
function gderiv(cache::AscherCache{iip, T}, gi, irow, zval, dgz, mode::Integer, izeta) where {iip, T}
    (; ncomp, bcjac) = cache
