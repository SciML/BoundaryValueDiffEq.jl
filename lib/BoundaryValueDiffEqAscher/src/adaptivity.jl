...
function mesh_selector!(cache::AscherCache{iip, T}, z, dmz, mesh, mesh_dt) where {iip, T}
    ...
    if (avrg < eps(T)) || (degequ >= 0.5)
        ...
        redistribute!(cache, n / 2, naccum, slope, accum)
    end
end

function redistribute!(cache::AscherCache{iip, T}, nold::Integer, naccum::Integer, slope, accum) where {iip, T}
...
function error_estimate!(cache::AscherCache)
    ...
    for i=n:-1:1        
        ...
        error[i] .= wgterr .* abs.(valstr[i][3] .- (isodd(i) ? valstr[Int((i+1)/2)][2] : valstr[Int(i/2)][4]))

        x = mesh[i] +  (mesh_dt[i]) / 3.0
        @views approx(cache, x, valstr[i][2])
        error[i] .= error[i] .+ wgterr .* abs.(valstr[i][2] .- (isodd(i) ? valstr[Int((i+1)/2)][1] : valstr[Int(i/2)][3]))
    end
